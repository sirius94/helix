...
impl helix_event::AsyncHook for SignatureHelpHandler {
    ...
    fn handle_event(
        ...
        match event {
            ...
            SignatureHelpEvent::RequestComplete { open: opened } => {
                if opened {
                    // don't cancel rerequest that were already triggered
...
pub(super) fn register_hooks(handlers: &Handlers) {
    ...
    let tx = handlers.signature_hints.clone();
    register_hook!(
        move |event: &mut PostInsertChar<'_, '_>| signature_hepl_post_insert_char_hook(&tx, event)
    );
